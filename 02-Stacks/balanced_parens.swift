...
# NOTE
This version of the problem allows for the reverse parens. `)(` is considered balanced

*/

var testString1 = "h((e))llo(world)()"
let characterOpposite: [Character:Character] = [
    "(": ")",
    ")": "(",
    "{": "}",
    "}": "{",
...
func checkParentheses(_ string: String) -> Bool {
    ...
    for letter in string {
        if characterOpposite[letter] == nil { continue }
        guard let lastLetter = stack.peek() else {
            stack.push(letter)
            continue
        }

        if characterOpposite[letter] == lastLetter {
            stack.pop()
            continue
        }
        stack.push(letter)
